                ...
                <div event="col-md-6">
                    <form event="ajax-update" data-reload="#page-header" method="post" action="{{ route('event.update', $event->id) }}" enctype="multipart/form-data">
                        ...
                        <div event="form-group row">
                            ...
                            <div event="col-lg-9">
                                <input name="title" value="{{ $event->title }}" required type="text" class="form-control" placeholder="Name of Event">
                            </div>
                        </div>
                            ...
                            <div event="col-lg-9">
                                <select data-placeholder="Select Teacher" event="form-control select-search" name="teacher_id" id="teacher_id">
                                    ...
                                    @foreach($teachers as $t)
                                        <option {{ $event->teacher_id == $t->id ? 'selected' : '' }} value="{{ Qs::hash($t->id) }}">{{ $t->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                      --}}
                        ...
                        {{-- <div event="form-group row">
                            ...
                            <div event="col-lg-9">
                                <input event="form-control" disabled="disabled" value="{{ json_encode($event->category()) }}" title="Event Type" type="text">
                            </div>
                        </div> --}}
